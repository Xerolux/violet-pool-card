import{LitElement as e,html as t,css as i}from"lit";import{property as a,customElement as s,state as o}from"lit/decorators.js";function r(e,t,i,a){var s,o=arguments.length,r=o<3?t:null===a?a=Object.getOwnPropertyDescriptor(t,i):a;if("object"==typeof Reflect&&"function"==typeof Reflect.decorate)r=Reflect.decorate(e,t,i,a);else for(var n=e.length-1;n>=0;n--)(s=e[n])&&(r=(o<3?s(r):o>3?s(t,i,r):s(t,i))||r);return o>3&&r&&Object.defineProperty(t,i,r),r}"function"==typeof SuppressedError&&SuppressedError;const n={off:{color:"#757575",icon:"mdi:power-off",label:"OFF"},on:{color:"#4CAF50",icon:"mdi:power-on",label:"ON"},auto:{color:"#2196F3",icon:"mdi:autorenew",label:"AUTO"},manual:{color:"#FF9800",icon:"mdi:hand-back-right",label:"MANUAL"},blocked:{color:"#FFC107",icon:"mdi:block-helper",label:"BLOCKED"},error:{color:"#F44336",icon:"mdi:alert-circle",label:"ERROR"},idle:{color:"#9E9E9E",icon:"mdi:sleep",label:"IDLE"},heat:{color:"#FF5722",icon:"mdi:fire",label:"HEAT"},heating:{color:"#FF5722",icon:"mdi:fire",label:"HEATING"},cool:{color:"#00BCD4",icon:"mdi:snowflake",label:"COOL"},cooling:{color:"#00BCD4",icon:"mdi:snowflake",label:"COOLING"}};let c=class extends e{constructor(){super(...arguments),this.pulse=!1,this.showIcon=!0}render(){const e=n[this.state]||n.off;return t` <div class="badge ${this.state} ${this.pulse?"pulse":""}" style="--badge-color: ${e.color}" > ${this.showIcon?t`<ha-icon icon="${this.icon||e.icon}"></ha-icon>`:""}
      ...
      ${s?t` <circle cx="24" cy="24" r="21" fill="none" stroke="${i}" stroke-width="3" stroke-dasharray="${33*e} 100" stroke-opacity="0.5" stroke-linecap="round" transform="rotate(-90 24 24)"/> `:""}
          ...
          <div class="speed-segments-container">
            <div class="speed-segments">
              ${[1,2,3].map(i=>t` <button class="speed-segment tooltip-wrap ${c===i?"seg-active":c>i?"seg-past":""}" style="--seg-color: ${y[i]}" @click="${t=>{t.stopPropagation(),new m(this.hass).setPumpSpeed(e.entity,i)}}" ><ha-icon icon="${w[i]}" style="--mdc-icon-size: 14px"></ha-icon><span>${b[i]}</span><div class="t-tip t-up"><div class="t-tip-title">${b[i]}-Geschwindigkeit</div><div class="t-tip-desc">${["","ECO-Modus – Langsame Filtration, sehr energieeffizient. Ideal für Nacht-Grundzirkulation.","Normal-Modus – Standard-Filtration für den täglichen Betrieb.","Boost-Modus – Maximale Leistung. Für Rückspülung, Staubsaugen oder schnelles Chemikalien-Mischen."][i]}</div></div></button> `)}
            </div>
            <button
              class="speed-off-btn tooltip-wrap ${0===c?"seg-active":""}"
              style="--seg-color: ${y[0]}"
              @click="${t=>{t.stopPropagation(),new m(this.hass).turnOff(e.entity)}}"
            >
          ...
          ${e.show_runtime&&d>0?t` <div class="info-row"><ha-icon icon="mdi:timer-outline"></ha-icon><span class="info-label">Runtime</span><span class="info-value">${v}</span></div> `:""}
        </div>
      </ha-card>
    `}async _handlePumpSpeedChange(e,t){const i=e.detail.value,a=new m(this.hass);await a.setPumpSpeed(t,i)}renderHeaterCard(e=this.config){const i=this.hass.states[e.entity],a=i.state,s=e.name||i.attributes.friendly_name||"Heater",o=this._getAccentColor("heater",e),r=f.getCurrentTemperature(i),n=f.getTargetTemperature(i),c=f.getMinTemperature(i)||18,l=f.getMaxTemperature(i)||35,d=i.attributes?.HEATERSTATE||"",p=i.attributes?.outside_temperature,h=i.attributes?.min_outside_temperature||14.5,v=d.includes("BLOCKED_BY_OUTSIDE_TEMP")||void 0!==p&&p<h,g=void 0!==r?x.getTemperatureColor(r):void 0,u=[{icon:"mdi:power-off",label:"OFF",action:async()=>{const t=new m(this.hass);await t.setHvacMode(e.entity,"off")},active:"off"===a,color:"#757575",confirmMessage:void 0},{icon:"mdi:autorenew",label:"AUTO",action:async()=>{const t=new m(this.hass);await t.setHvacMode(e.entity,"auto")},active:"auto"===a,color:"#2196F3"},{icon:"mdi:fire",label:"HEAT",action:async()=>{const t=new m(this.hass);await t.setHvacMode(e.entity,"heat")},active:"heat"===a||"heating"===a,color:"#FF5722"}],b="heating"===a||"heat"===a,y=void 0!==r?this._getValuePercent(r,c,l):void 0,w=void 0!==n?this._getValuePercent(n,c,l):void 0;return t` <ha-card class="${this._getCardClasses(b,e)}" style="--card-accent: ${o}" @click="${()=>this._showMoreInfo(e.entity)}" ><div class="accent-bar"></div><div class="card-content"><div class="header"><div class="header-icon ${b?"icon-active":""}" style="--icon-accent: ${o}">${e.icon?t`<ha-icon icon="${e.icon}" class="${b?"heater-active":""}"></ha-icon>`:($=b,_=o,t` <svg viewBox="0 0 48 48" style="width:100%;height:100%;display:block;overflow:visible" xmlns="http://www.w3.org/2000/svg"><circle cx="24" cy="24" r="22" fill="${_}" fill-opacity="${$?.14:.07}"/><circle cx="24" cy="24" r="21" fill="none" stroke="${_}" stroke-width="1.5" stroke-opacity="0.38"/><path d="M24,8 C17,15 12,20 14,27 C15,33 20,38 24,40 C28,38 33,33 34,27 C36,20 31,15 24,8Z" fill="${_}" fill-opacity="${$?.88:.5}" style="${$?"animation:flicker 1.9s ease-in-out infinite;transform-origin:24px 40px":""}"/><path d="M24,19 C21,23 19,26 20,30 C21,34 22.5,37 24,38 C25.5,37 27,34 28,30 C29,26 27,23 24,19Z" fill="white" fill-opacity="${$?.55:.2}" style="${$?"animation:flicker 1.4s ease-in-out infinite 0.2s;transform-origin:24px 38px":""}"/><ellipse cx="24" cy="39" rx="8" ry="2.5" fill="${_}" fill-opacity="${$?.28:.1}"/></svg>`)}</div><div class="header-info"><span class="name">${s}</span><span class="header-subtitle">${this._getFriendlyState(a)}</span></div> ${e.show_state?t`<status-badge .state="${a}"></status-badge>`:""}
          </div>

          ${void 0!==r?t` <div class="temp-hero tooltip-wrap" style="--temp-color: ${g?.color||"var(--vpc-primary)"}; position: relative;"><div class="temp-hero-main"><span class="temp-hero-value">${r.toFixed(1)}</span><span class="temp-hero-unit">°C</span></div> ${void 0!==n?t`
                ...
                <div class="t-tip">
                  ...
                  <div class="t-tip-desc">Aktuelle Pooltemperatur${void 0!==n?`. Ziel: ${n.toFixed(1)}°C`:""}. ${r<24?"Noch kalt zum Schwimmen.":r<=30?"Ideale Badetemperatur.":"Etwas warm – Heizung prüfen."}</div>
                  <div class="t-tip-ideal"><ha-icon icon="mdi:target"></ha-icon>24°C – 30°C Komfort</div>
                </div>
                </div>
                ${void 0!==y?t` <div class="temp-range-bar"><div class="temp-range-track"><div class="temp-range-fill" style="width: ${y}%; background: ${g?.color||o}"></div> ${void 0!==w?t`<div class="temp-range-target" style="left: ${w}%"></div>`:""}
                        </div>
          ...
          ${void 0!==p?t` <div class="info-row tooltip-wrap ${v?"info-row-warning":""}" style="position:relative"><ha-icon icon="mdi:thermometer"></ha-icon><span class="info-label">Außentemperatur</span><span class="info-value">${p.toFixed(1)}°C</span> ${v?t`<span class="info-badge warning">Min ${h}°C</span>`:""}
                <div class="t-tip">
                  <div class="t-tip-title"><ha-icon icon="mdi:thermometer-alert"></ha-icon>Außentemperatur</div>
                  <div class="t-tip-desc">${v?`Heizung gesperrt! Außentemperatur (${p.toFixed(1)}°C) liegt unter dem Minimum von ${h}°C. Frostschutz aktiv.`:`Aktuelle Außentemperatur. Heizung wird bei unter ${h}°C Außentemperatur gesperrt (Frostschutz).`}</div>
                  ${v?t`<div class="t-tip-warn"><ha-icon icon="mdi:snowflake-alert"></ha-icon>Heizung gesperrt</div>`:""}
                </div>
                </div>
              `:""}
    ...
    `;var $,_}async _handleTemperatureChange(e,t){const i=e.detail.value,a=new m(this.hass);await a.setTemperature(t,i)}renderSolarCard(e=this.config){const i=this.hass.states[e.entity],a=i.state,s=e.name||i.attributes.friendly_name||"Solar",o=this._getAccentColor("solar",e),r=f.getCurrentTemperature(i),n=f.getTargetTemperature(i),c=f.getMinTemperature(i)||18,l=f.getMaxTemperature(i)||32,d=i.attributes?.absorber_temperature,p=void 0!==d&&void 0!==r?d-r:void 0,h=i.attributes?.SOLARSTATE||"",v=[{icon:"mdi:power-off",label:"OFF",action:async()=>{const t=new m(this.hass);await t.setHvacMode(e.entity,"off")},active:"off"===a,color:"#757575"},{icon:"mdi:autorenew",label:"AUTO",action:async()=>{const t=new m(this.hass);await t.setHvacMode(e.entity,"auto")},active:"auto"===a,color:"#2196F3"},{icon:"mdi:sun-thermometer",label:"ON",action:async()=>{const t=new m(this.hass);await t.setHvacMode(e.entity,"heat")},active:"heat"===a||"heating"===a,color:"#FF9800"}],g="heating"===a||"heat"===a,u=void 0!==r?this._getValuePercent(r,c,l):void 0,x=void 0!==n?this._getValuePercent(n,c,l):void 0;return t` <ha-card class="${this._getCardClasses(g,e)}" style="--card-accent: ${o}" @click="${()=>this._showMoreInfo(e.entity)}" ><div class="accent-bar"></div><div class="card-content"><div class="header"><div class="header-icon ${g?"icon-active":""}" style="--icon-accent: ${o}">${e.icon?t`<ha-icon icon="${e.icon}" class="${g?"solar-active":""}"></ha-icon>`:(b=g,y=o,t` <svg viewBox="0 0 48 48" style="width:100%;height:100%;display:block;overflow:visible" xmlns="http://www.w3.org/2000/svg"><circle cx="24" cy="24" r="22" fill="${y}" fill-opacity="${b?.13:.07}"/><g style="transform-origin:24px 24px;animation:${b?"spin-slow 14s linear infinite":"none"}"> ${[0,45,90,135,180,225,270,315].map(e=>t`
          <rect x="22.5" y="3" width="3" height="5.5" rx="1.5"
                fill="${y}" fill-opacity="${b?.75:.3}"
                style="transform:rotate(${e}deg);transform-origin:24px 24px"/>
        `)}
      </g>
      ${b?t` <circle cx="24" cy="24" r="14" fill="none" stroke="${y}" stroke-width="1.5" stroke-opacity="0.35" style="animation:breathe 2.8s ease-in-out infinite"/> `:""}
      <circle cx="24" cy="24" r="10.5" fill="${y}" fill-opacity="${b?.9:.45}"/>
      <circle cx="24" cy="24" r="7" fill="white" fill-opacity="${b?.5:.22}"/>
    </svg>`)}</div><div class="header-info"><span class="name">${s}</span><span class="header-subtitle">${this._getFriendlyState(a)}</span></div> ${e.show_state?t`<status-badge .state="${a}"></status-badge>`:""}
          </div>
          ...
          <div class="solar-temps">
            ...
            <div class="solar-temp-comparison">
              ...
              ${void 0!==p?t` <div class="solar-delta-badge tooltip-wrap ${p>=3?"delta-great":p>0?"delta-ok":"delta-low"}" style="position:relative"><ha-icon icon="${p>=0?"mdi:trending-up":"mdi:trending-down"}" style="--mdc-icon-size: 16px"></ha-icon><span>${p>0?"+":""}${p.toFixed(1)}°C</span><div class="t-tip"><div class="t-tip-title"><ha-icon icon="mdi:delta"></ha-icon>Temperaturdifferenz</div><div class="t-tip-desc">Differenz zwischen Absorber (Kollektor) und Pool. ${p>=3?"Sehr gute Solarbedingungen! Heizung wird aktiv.":p>0?"Solarbeheizung möglich, aber noch schwach.":"Absorber kälter als Pool – keine Solarheizung möglich."}</div><div class="t-tip-ideal"><ha-icon icon="mdi:target"></ha-icon>&gt; 3°C für aktive Solarheizung</div></div></div> `:""}
          ...
          ${e.show_controls?t` ${void 0!==n?t`
                      ...
                      ></slider-control>
                    `:""}
                <quick-actions .actions="${v}"></quick-actions>
              `:""}
        </div>
      </ha-card>
    `;var b,y}renderDosingCard(e=this.config){const i=this.hass.states[e.entity],a=i.state,s=e.name||i.attributes.friendly_name||"Dosing",o=this._getAccentColor("dosing",e),r=e.dosing_type||this._detectDosingType(e.entity),n=Object.keys(i.attributes||{}).find(e=>e.includes("DOS_")&&e.includes("_STATE")),c=n?i.attributes[n]:[];let l,d,p,h,v="";if("chlorine"===r){const e=this._getEntityId("orp_value_entity","sensor","orp_value"),t=this.hass.states[e];l=t?parseFloat(t.state):void 0;const i=this._getEntityId("target_orp_entity","number","target_orp"),a=this.hass.states[i];d=a?parseFloat(a.state):void 0,p=a?.attributes?.min||600,h=a?.attributes?.max||800,v="mV"}else if("ph_minus"===r||"ph_plus"===r){const e=this._getEntityId("ph_value_entity","sensor","ph_value"),t=this.hass.states[e];l=t?parseFloat(t.state):void 0;const i=this._getEntityId("target_ph_entity","number","target_ph"),a=this.hass.states[i];d=a?parseFloat(a.state):void 0,p=a?.attributes?.min||6.8,h=a?.attributes?.max||7.8,v=""}const g=i.attributes?.dosing_volume_24h,u=[{icon:"mdi:power-off",label:"OFF",action:async()=>{const t=new m(this.hass);await t.turnOff(e.entity)},active:"off"===a,color:"#757575"},{icon:"mdi:autorenew",label:"AUTO",action:async()=>{const t=new m(this.hass);await t.turnOn(e.entity)},active:"on"===a||"auto"===a,color:"#2196F3"},{icon:"mdi:play-circle",label:"Dose 30s",action:async()=>{const t=new m(this.hass);await t.manualDosing(e.entity,30)},color:"#4CAF50",confirmMessage:"Start manual dosing for 30 seconds?"},{icon:"mdi:play-speed",label:"Dose 60s",action:async()=>{const t=new m(this.hass);await t.manualDosing(e.entity,60)},color:"#FF9800",confirmMessage:"Start manual dosing for 60 seconds?"}],f="on"===a&&Array.isArray(c)&&c.some(e=>e.includes("ACTIVE")),b=void 0!==l?"chlorine"===r?x.getOrpColor(l,d):x.getPhColor(l,d):void 0,y=void 0!==l&&void 0!==p&&void 0!==h?this._getValuePercent(l,p,h):void 0,w=void 0!==d&&void 0!==p&&void 0!==h?this._getValuePercent(d,p,h):void 0,$="chlorine"===r?0:1,_="chlorine"===r?"ORP":"ph_minus"===r||"ph_plus"===r?"pH":"Floc",k=b?"chlorine"===r?l<(d??650)?"Low":l>(d??750)?"High":"Optimal":l<7?"Acidic":l>7.4?"Alkaline":"Optimal":"";return t` <ha-card class="${this._getCardClasses(f,e)}" style="--card-accent: ${o}" @click="${()=>this._showMoreInfo(e.entity)}" ><div class="accent-bar"></div><div class="card-content"><div class="header"><div class="header-icon ${f?"icon-active":""}" style="--icon-accent: ${o}"><ha-icon icon="${e.icon||this._getDosingIcon(r)}" class="${f?"dosing-active":""}" ></ha-icon></div><div class="header-info"><span class="name">${s}</span><span class="header-subtitle">${this._getFriendlyState(a)}</span></div> ${e.show_state?t`<status-badge .state="${a}" .pulse="${f}"></status-badge>`:""}
          </div>

          ${void 0!==l?t` <!-- Dosing value hero with progress bar --><div class="dosing-value-block tooltip-wrap" style="position:relative"><div class="dosing-value-row"><div class="dosing-value-main" style="color: ${b?.color||"var(--vpc-text)"}"><span class="dosing-label-tag">${_}</span><span class="dosing-current-value">${l.toFixed($)}</span><span class="dosing-current-unit">${v}</span></div><div class="dosing-status-pill" style="background: ${b?.color?b.color+"18":"rgba(0,0,0,0.05)"}; color: ${b?.color||"var(--vpc-text-secondary)"}"> ${k} </div></div><div class="t-tip t-up"><div class="t-tip-title"><ha-icon icon="${this._getDosingIcon(r)}"></ha-icon>${"chlorine"===r?"ORP – Chlorwirksamkeit":"pH-Wert"}</div><div class="t-tip-desc">${"chlorine"===r?`Redoxpotential (ORP) zeigt, wie wirksam das Chlor Keime abtötet. Aktuell: ${l.toFixed(0)} mV${void 0!==d?`, Ziel: ${d.toFixed(0)} mV`:""}.`:`pH-Wert des Poolwassers. Aktuell: ${l.toFixed(1)}${void 0!==d?", Ziel: "+d.toFixed(1):""}.`}</div>
                <div class="t-tip-ideal"><ha-icon icon="mdi:target"></ha-icon>${"chlorine"===r?"650 – 750 mV Optimal":"7.0 – 7.4 Optimal"}</div>
              </div> ${void 0!==y?t` <div class="chem-range-bar"><div class="chem-range-track"><div class="chem-range-fill" style="width: ${y}%; background: ${b?.color||o}"></div> ${void 0!==w?t` <div class="chem-range-target" style="left: ${w}%"><div class="chem-target-line"></div><div class="chem-target-label">${d.toFixed($)}${v}</div></div> `:""}
                          </div>
                          <div class="chem-range-labels">
                            <span>${p.toFixed($)}${v}</span>
                            <span>${h.toFixed($)}${v}</span>
                          </div>
                        </div>
                      `:""}
                </div>
          ...
          ${e.show_history&&void 0!==g?t` <div class="info-row"><ha-icon icon="mdi:chart-line"></ha-icon><span class="info-label">Last 24h</span><span class="info-value">${g}ml</span></div> `:""}
        </div>
      </ha-card>
    `}_detectDosingType(e){return e.includes("_cl")?"chlorine":e.includes("_phm")?"ph_minus":e.includes("_php")?"ph_plus":e.includes("_floc")?"flocculant":"chlorine"}_getDosingIcon(e){switch(e){case"chlorine":default:return"mdi:flask-outline";case"ph_minus":return"mdi:flask-minus";case"ph_plus":return"mdi:flask-plus";case"flocculant":return"mdi:flask"}}renderOverviewCard(e=this.config){const i=e.name||"Pool Status",a=this._getAccentColor("overview",e),s=this._getEntityId("pump_entity","switch","pump",0),o=this._getEntityId("heater_entity","climate","heater",1),r=this._getEntityId("solar_entity","climate","solar",2),n=this._getEntityId("chlorine_entity","switch","dos_1_cl",3),c=this._getEntityId("ph_minus_entity","switch","dos_2_phm",4),l=this.hass.states[s],d=this.hass.states[o],p=this.hass.states[r],h=this.hass.states[n],v=this.hass.states[c],g=this._getEntityId("pool_temp_entity","sensor","temperature",5),u=this._getEntityId("ph_value_entity","sensor","ph_value",6),m=this._getEntityId("orp_value_entity","sensor","orp_value",7),b=this.hass.states[g],y=this.hass.states[u],w=this.hass.states[m],$=b?parseFloat(b.state):void 0,_=y?parseFloat(y.state):void 0,k=w?parseFloat(w.state):void 0,C=void 0!==$?x.getTemperatureColor($):void 0,F=void 0!==_?x.getPhColor(_):void 0,z=void 0!==k?x.getOrpColor(k):void 0,A=void 0===(S=_)||isNaN(S)?"unknown":S<7||S>7.4?"warning":"ok";var S;const P=void 0===(E=k)||isNaN(E)?"unknown":E<650?"warning":E>750?"high":"ok";var E;const T=[];if(l){const e=f.parsePumpState(l.attributes?.PUMPSTATE||"");T.push({icon:"mdi:pump",name:"Pump",status:e.status||l.state,state:l.state,entityId:s})}if(d){const e=f.parseHeaterState(d.attributes?.HEATERSTATE||"");T.push({icon:"mdi:radiator",name:"Heater",status:e.status||d.state,state:d.state,entityId:o})}if(p){const e=f.parseSolarState(p.attributes?.SOLARSTATE||"");T.push({icon:"mdi:solar-power",name:"Solar",status:e.status||p.state,state:p.state,entityId:r})}if(h){const e=h.attributes?.DOS_1_CL_STATE||[],t=Array.isArray(e)&&e.length>0?f.formatSnakeCase(e[0]):h.state;T.push({icon:"mdi:flask-outline",name:"Chlorine",status:t,state:h.state,entityId:n})}if(v){const e=v.attributes?.DOS_2_PHM_STATE||[],t=Array.isArray(e)&&e.length>0?f.formatSnakeCase(e[0]):v.state;T.push({icon:"mdi:flask-minus",name:"pH-",status:t,state:v.state,entityId:c})}const M=this._getEntityId("cover_entity","cover","cover"),D=this.hass.states[M];if(D){const e=D.attributes?.current_position,t="opening"===D.state||"closing"===D.state;T.push({icon:"open"===D.state?"mdi:window-shutter-open":"mdi:window-shutter",name:D.attributes.friendly_name||"Abdeckung",status:void 0!==e?`${Math.round(e)}%${t?"opening"===D.state?" ↑":" ↓":""}`:"open"===D.state?"Offen":"Zu",state:"open"===D.state?"on":"closed"===D.state?"off":"auto",entityId:M})}const I=this._getEntityId("light_entity","light","light"),O=this.hass.states[I];if(O){const e=O.attributes?.brightness;T.push({icon:"on"===O.state?"mdi:lightbulb-on":"mdi:lightbulb-off-outline",name:O.attributes.friendly_name||"Beleuchtung",status:"on"===O.state?"An"+(void 0!==e?` · ${Math.round(e/255*100)}%`:""):"Aus",state:O.state,entityId:I})}const V=this._getEntityId("filter_entity","sensor","filter_pressure"),H=this.hass.states[V];if(H){const e=parseFloat(H.state);T.push({icon:"mdi:filter",name:H.attributes.friendly_name||"Filter",status:isNaN(e)?H.state:e.toFixed(2)+" bar",state:!isNaN(e)&&e>1.2?"auto":"on",entityId:V})}const B=[];if("warning"===P&&B.push("ORP too low - Check chlorine dosing"),"high"===P&&B.push("ORP too high - Stop chlorine dosing"),"warning"===A&&B.push("pH out of range - Check dosing"),l?.attributes?.PUMPSTATE?.includes("ANTI_FREEZE")){const e=d?.attributes?.outside_temperature;B.push("Frost protection active"+(e?` (${e.toFixed(1)}°C)`:""))}const N=T.some(e=>["on","auto","heat","heating"].includes(e.state)),R=T.filter(e=>["on","auto","heat","heating"].includes(e.state)).length,L=void 0!==$?this._getValuePercent($,18,35):void 0,j=void 0!==_?this._getValuePercent(_,6.5,8):void 0,U=void 0!==k?this._getValuePercent(k,500,900):void 0,q=this._getValuePercent(7,6.5,8),W=this._getValuePercent(7.4,6.5,8),Z=this._getValuePercent(650,500,900),K=this._getValuePercent(750,500,900);return t` <ha-card class="${this._getCardClasses(N,e)}" style="--card-accent: ${a}" ><div class="accent-bar"></div><div class="card-content"><!-- Header --><div class="header"><div class="header-icon ${N?"icon-active":""}" style="--icon-accent: ${a}"><ha-icon icon="mdi:pool"></ha-icon></div><div class="header-info"><span class="name">${i}</span><span class="header-subtitle"> ${N?`${R} device${1!==R?"s":""} active`:"All systems idle"}
              </span>
            </div>
            ${B.length>0?t`<div class="overview-warning-badge">${B.length}</div>`:N?t`<div class="overview-active-dot"></div>`:""}
          </div>

          <!-- Water Chemistry - Apple Health style metric tiles -->
          <div class="chemistry-grid">
            ${void 0!==$?t` <div class="chemistry-card tooltip-wrap" style="--chem-color: ${C?.color||"#4CAF50"}" @click="${e=>{e.stopPropagation(),this._showMoreInfo(g)}}"><div class="chem-icon-wrap"><ha-icon icon="mdi:thermometer-water"></ha-icon></div><span class="chemistry-val">${$.toFixed(1)}°</span><span class="chemistry-unit">°C</span><span class="chemistry-label">${$<22?"Kalt":$<26?"OK":$<=30?"Ideal":"Warm"}</span> ${void 0!==L?t`<div class="chem-mini-bar"><div class="chem-mini-ideal" style="left:${this._getValuePercent(24,18,35)}%;width:${this._getValuePercent(30,18,35)-this._getValuePercent(24,18,35)}%"></div><div class="chem-mini-fill" style="width: ${L}%; background: ${C?.color||"#4CAF50"}"></div></div>`:""}
                  <div class="t-tip">
                    <div class="t-tip-title"><ha-icon icon="mdi:thermometer-water"></ha-icon>Wassertemperatur</div>
                ...
                `:""}
            ${void 0!==_?t` <div class="chemistry-card tooltip-wrap" style="--chem-color: ${F?.color||"#4CAF50"}" @click="${e=>{e.stopPropagation(),this._showMoreInfo(u)}}"><div class="chem-icon-wrap"><ha-icon icon="mdi:ph"></ha-icon></div><span class="chemistry-val">${_.toFixed(1)}</span><span class="chemistry-unit">pH</span><span class="chemistry-label">${"ok"===A?"Optimal":"Achtung"}</span> ${void 0!==j?t`
                          <div class="chem-mini-bar">
                            <div class="chem-mini-ideal" style="left: ${q}%; width: ${W-q}%"></div>
                            <div class="chem-mini-fill" style="width: ${j}%; background: ${F?.color||"#4CAF50"}"></div>
                          </div>
                        `:""}
                  <div class="t-tip">
                    ...
                    ${_<7?t`<div class="t-tip-warn"><ha-icon icon="mdi:arrow-up"></ha-icon>pH+ zugeben zum Erhöhen</div>`:_>7.4?t`<div class="t-tip-warn"><ha-icon icon="mdi:arrow-down"></ha-icon>pH- zugeben zum Senken</div>`:""}
                  </div>
                  </div>
                `:""}
            ${void 0!==k?t` <div class="chemistry-card tooltip-wrap" style="--chem-color: ${z?.color||"#4CAF50"}" @click="${e=>{e.stopPropagation(),this._showMoreInfo(m)}}"><div class="chem-icon-wrap"><ha-icon icon="mdi:lightning-bolt"></ha-icon></div><span class="chemistry-val">${k.toFixed(0)}</span><span class="chemistry-unit">mV</span><span class="chemistry-label">${"ok"===P?"Optimal":"warning"===P?"Niedrig":"Hoch"}</span> ${void 0!==U?t`
                          <div class="chem-mini-bar">
                            <div class="chem-mini-ideal" style="left: ${Z}%; width: ${K-Z}%"></div>
                            <div class="chem-mini-fill" style="width: ${U}%; background: ${z?.color||"#4CAF50"}"></div>
                          </div>
                        `:""}
                  <div class="t-tip">
                    ...
                    ${k<650?t`<div class="t-tip-warn"><ha-icon icon="mdi:flask-outline"></ha-icon>Chlordosierung erhöhen</div>`:k>750?t`<div class="t-tip-warn"><ha-icon icon="mdi:flask-outline"></ha-icon>Chlordosierung reduzieren</div>`:""}
                  </div>
                  </div>
                `:""}
          </div>
          ...
          ${T.length>0?t` <div class="overview-section"><div class="section-title"><span>Devices</span><span class="section-count">${T.length}</span></div><div class="device-list"> ${T.map(e=>t`
          ...
          <!-- Warnings / All OK -->
          ${B.length>0?t` <div class="overview-section"><div class="section-title warning-title"><ha-icon icon="mdi:alert-outline" style="--mdc-icon-size: 14px"></ha-icon><span>Alerts</span></div><div class="warning-list"> ${B.map(e=>t`
    ...
    `}renderDetailsCard(e=this.config){const i=e.entities||[],a=e.icon;return i.length?t` <ha-card class="theme-${e.theme||"luxury"} size-${e.size||"medium"}"><div class="card-content" style="padding: var(--vpc-spacing, 16px);"> ${t`
          <div class="card-header" style="display:flex;align-items:center;gap:12px;margin-bottom:16px;">
            ${a?t`<div class="header-icon" style="width:38px;height:38px;border-radius:10px;background:var(--vpc-surface);display:flex;align-items:center;justify-content:center;color:var(--vpc-primary);"><ha-icon icon="${a}"></ha-icon></div>`:""}
            <div class="name" style="font-size:16px;font-weight:600;color:var(--vpc-text);margin:0;">${e.name||e.title||"Details"}</div>
          </div>
          `}
          <div class="device-list">
            ${i.map(e=>{let i="",a="",s="";"string"==typeof e?i=e:(i=e.entity,a=e.name,s=e.icon);const o=this.hass.states[i];if(!o)return t` <div class="device-row"><div class="device-info"><div class="device-name">${i}</div><div class="device-status" style="color:var(--vpc-danger, #FF3B30)">Not found</div></div></div> `;const r=i.split(".")[0],n=a||o.attributes.friendly_name||i;let c=s||o.attributes.icon;c||(c="switch"===r||"light"===r||"binary_sensor"===r?"on"===o.state?"mdi:check-circle":"mdi:circle-outline":"sensor"===r&&"temperature"===o.attributes.device_class?"mdi:thermometer":"sensor"===r&&"%"===o.attributes.unit_of_measurement?"mdi:water-percent":"mdi:information-outline");const l=["switch","light","input_boolean"].includes(r),d="on"===o.state,p=l;let h=o.state;return o.attributes.unit_of_measurement&&(h+=" "+o.attributes.unit_of_measurement),t` <div class="device-row" @click="${()=>{if(p)this.hass.callService("homeassistant","toggle",{entity_id:i});else{const e=new Event("hass-more-info",{bubbles:!0,composed:!0});e.detail={entityId:i},this.dispatchEvent(e)}}}"><div class="device-icon-wrap ${d&&(l||"binary_sensor"===r)?"device-icon-active":""}"><ha-icon icon="${c}"></ha-icon></div><div class="device-info"><div class="device-name">${n}</div><div class="device-status">${h}</div></div> ${l?t`
    ...
    `}renderChemicalCard(e=this.config){const i=e.name||"Wasserchemie",a="#4CAF50",s=this._getEntityId("pool_temp_entity","sensor","temperature",5),o=this._getEntityId("ph_value_entity","sensor","ph_value",6),r=this._getEntityId("orp_value_entity","sensor","orp_value",7),n=this._getEntityId("target_ph_entity","number","target_ph"),c=this._getEntityId("target_orp_entity","number","target_orp"),l=this.hass.states[s],d=this.hass.states[o],p=this.hass.states[r],h=this.hass.states[n],v=this.hass.states[c],g=l?parseFloat(l.state):void 0,u=d?parseFloat(d.state):void 0,m=p?parseFloat(p.state):void 0,f=h?parseFloat(h.state):7.2,b=v?parseFloat(v.state):700,y=void 0!==g?x.getTemperatureColor(g):void 0,w=void 0!==u?x.getPhColor(u,f):void 0,$=void 0!==m?x.getOrpColor(m,b):void 0,_=void 0!==u&&u>=7&&u<=7.4,k=void 0!==m&&m>=650&&m<=750,C=[!_,!k].filter(e=>void 0!==u&&void 0!==m&&e).length,F=0===C?"Optimal":1===C?"Achtung":"Eingriff nötig",z=0===C?"var(--vpc-success, #34C759)":1===C?"var(--vpc-warning, #FF9F0A)":"var(--vpc-danger, #FF3B30)",A=0===C?"mdi:water-check":1===C?"mdi:alert":"mdi:alert-circle",S=void 0!==g?this._getValuePercent(g,18,35):void 0,P=void 0!==u?this._getValuePercent(u,6.5,8):void 0,E=void 0!==m?this._getValuePercent(m,500,900):void 0,T=this._getValuePercent(f,6.5,8),M=this._getValuePercent(b,500,900),D=this._getValuePercent(7,6.5,8),I=this._getValuePercent(7.4,6.5,8),O=this._getValuePercent(650,500,900),V=this._getValuePercent(750,500,900);return t` <ha-card class="${this._getCardClasses(0===C&&(void 0!==u||void 0!==m),e)}" style="--card-accent: ${a}"><div class="accent-bar"></div><div class="card-content"><div class="header"><div class="header-icon" style="--icon-accent: ${a}"><ha-icon icon="${e.icon||"mdi:water-check"}"></ha-icon></div><div class="header-info"><span class="name">${i}</span><span class="header-subtitle" style="color: ${z}">${F}</span></div><div class="chem-overall-badge" style="background: color-mix(in srgb, ${z} 12%, transparent); color: ${z}; border: 1px solid color-mix(in srgb, ${z} 25%, transparent);"><ha-icon icon="${A}" style="--mdc-icon-size: 14px"></ha-icon><span>${F}</span></div></div> ${void 0!==g?t`
            <div class="chem-section tooltip-wrap">
              <div class="chem-section-header">
                <ha-icon icon="mdi:thermometer-water" style="--mdc-icon-size: 15px; color: ${y?.color||"var(--vpc-text-secondary)"}"></ha-icon>
                <span>Wassertemperatur</span>
                <span class="chem-section-status" style="color: ${y?.color}">${N=g,void 0===N?"":N<20?"Sehr kalt":N<24?"Kühl":N<=28?"Perfekt":N<=32?"Angenehm warm":"Zu warm"}</span>
              </div>
              <div class="chem-big-value" style="color: ${y?.color||"var(--vpc-text)"}">
                <span class="chem-big-num">${g.toFixed(1)}</span>
                <span class="chem-big-unit">°C</span>
              </div>
              ${void 0!==S?t` <div class="chem-gauge-bar"><div class="chem-gauge-track"><div class="chem-gauge-fill" style="width: ${S}%; background: ${y?.color||a}"></div><div class="chem-gauge-zone" style="left: ${this._getValuePercent(24,18,35)}%; width: ${this._getValuePercent(30,18,35)-this._getValuePercent(24,18,35)}%"></div></div><div class="chem-gauge-labels"><span>18°C</span><span class="chem-zone-label">Komfortzone 24–30°C</span><span>35°C</span></div></div> `:""}
              <div class="t-tip">
                <div class="t-tip-title"><ha-icon icon="mdi:thermometer-water"></ha-icon>Wassertemperatur</div>
          ...
          <div class="chem-dual-grid">
            ${void 0!==u?t` <div class="chem-metric-card tooltip-wrap" style="--chem-color: ${w?.color||"#4CAF50"}" @click="${e=>{e.stopPropagation(),this._showMoreInfo(o)}}"><div class="chem-metric-header"><ha-icon icon="mdi:ph"></ha-icon><span>pH-Wert</span></div><div class="chem-metric-value">${u.toFixed(1)}</div><div class="chem-metric-status">${B=u,void 0===B?"Unbekannt":B<6.8?"Zu sauer":B<7?"Leicht sauer":B<=7.4?"Optimal":B<=7.6?"Leicht basisch":"Zu basisch"}</div> ${void 0!==P?t`
                  <div class="chem-metric-bar">
                    <div class="chem-metric-track">
                      <div class="chem-metric-ideal" style="left: ${D}%; width: ${I-D}%"></div>
                      <div class="chem-metric-fill" style="width: ${P}%"></div>
                      <div class="chem-metric-target" style="left: ${T}%"></div>
                    </div>
                ...
                <div class="t-tip">
                  <div class="t-tip-title"><ha-icon icon="mdi:ph"></ha-icon>pH-Wert</div>
                  <div class="t-tip-desc">Säuregehalt des Wassers. Zu niedrig reizt Haut und Augen. Zu hoch reduziert die Chlorwirksamkeit. Ziel: ${f.toFixed(1)}</div>
                  <div class="t-tip-ideal"><ha-icon icon="mdi:target"></ha-icon>7.0 – 7.4 Optimal</div>
                  ${_?"":t`<div class="t-tip-warn"><ha-icon icon="mdi:flask"></ha-icon>${u<7?"pH+ zugeben":"pH- zugeben"}</div>`}
                </div>
              </div>
            `:""}

            ${void 0!==m?t` <div class="chem-metric-card tooltip-wrap" style="--chem-color: ${$?.color||"#4CAF50"}" @click="${e=>{e.stopPropagation(),this._showMoreInfo(r)}}"><div class="chem-metric-header"><ha-icon icon="mdi:lightning-bolt"></ha-icon><span>ORP</span></div><div class="chem-metric-value">${m.toFixed(0)}</div><div class="chem-metric-unit">mV</div><div class="chem-metric-status">${H=m,void 0===H?"Unbekannt":H<600?"Zu niedrig":H<650?"Niedrig":H<=750?"Optimal":H<=800?"Erhöht":"Zu hoch"}</div> ${void 0!==E?t`
                  <div class="chem-metric-bar">
                    <div class="chem-metric-track">
                      <div class="chem-metric-ideal" style="left: ${O}%; width: ${V-O}%"></div>
                      <div class="chem-metric-fill" style="width: ${E}%"></div>
                      <div class="chem-metric-target" style="left: ${M}%"></div>
                    </div>
                ...
                <div class="t-tip">
                  <div class="t-tip-title"><ha-icon icon="mdi:lightning-bolt"></ha-icon>ORP – Desinfektionskraft</div>
                  <div class="t-tip-desc">Redoxpotential misst, wie wirksam das Chlor Keime abtötet. Niedriger ORP = unzureichende Desinfektion. Ziel: ${b.toFixed(0)} mV</div>
                  <div class="t-tip-ideal"><ha-icon icon="mdi:target"></ha-icon>650 – 750 mV Optimal</div>
                  ${k?"":t`<div class="t-tip-warn"><ha-icon icon="mdi:flask-outline"></ha-icon>${m<650?"Chlor erhöhen":"Chlor reduzieren"}</div>`}
                </div>
              </div>
            `:""}
          </div>

          ${C>0?t` <div class="chem-recommendations"> ${void 0===u||_?"":t`
                <div class="chem-rec-row" style="--rec-color: ${w?.color||"var(--vpc-warning)"}">
                  <ha-icon icon="mdi:ph"></ha-icon>
                  <span>${u<7?"pH zu sauer: pH+ Mittel zugeben":"pH zu basisch: pH- Mittel zugeben"}</span>
                </div>
              `}
              ${void 0===m||k?"":t` <div class="chem-rec-row" style="--rec-color: ${$?.color||"var(--vpc-warning)"}"><ha-icon icon="mdi:flask-outline"></ha-icon><span>${m<650?"ORP zu niedrig: Chlordosierung erhöhen":"ORP zu hoch: Chlordosierung reduzieren"}</span></div> `}
            </div>
          `:t` <div class="all-ok-display"><ha-icon icon="mdi:water-check" style="--mdc-icon-size: 18px"></ha-icon><span>Wasserqualität optimal</span></div> `}
        </div>
      </ha-card>
    `;var H,B,N}renderSensorCard(e=this.config){const i=this.hass.states[e.entity],a=i.state,s=e.name||i.attributes.friendly_name||"Sensor",o=i.attributes.unit_of_measurement||"",r=e.accent_color||this._getAccentColor("overview",e),n=parseFloat(a),c=!isNaN(n),l=i.attributes.device_class,d={temperature:{title:"Temperatur",desc:"Aktuelle Temperaturmessung des Sensors.",ideal:"24°C – 30°C (Pool)"},ph:{title:"pH-Wert",desc:"Säuregehalt des Poolwassers.",ideal:"7.0 – 7.4"},voltage:{title:"ORP / Spannung",desc:"Redoxpotential – Desinfektionskraft des Wassers.",ideal:"650 – 750 mV"},humidity:{title:"Luftfeuchtigkeit",desc:"Relative Luftfeuchtigkeit in Prozent."},pressure:{title:"Druck",desc:"Aktueller Druckwert."}}[l]||{title:s,desc:"Sensorwert aus dem Violet Pool System."},p=c?n.toFixed(n%1==0?0:1):a;return t` <ha-card class="${this._getCardClasses(!1,e)}" style="--card-accent: ${r}" @click="${()=>this._showMoreInfo(e.entity)}"><div class="accent-bar"></div><div class="card-content"><div class="header"><div class="header-icon" style="--icon-accent: ${r}"><ha-icon icon="${e.icon||("temperature"===l?"mdi:thermometer":"ph"===l?"mdi:ph":"mdi:gauge")}"></ha-icon></div><div class="header-info"><span class="name">${s}</span><span class="header-subtitle">${l?l.charAt(0).toUpperCase()+l.slice(1):"Sensor"}</span></div></div><div class="sensor-value-display tooltip-wrap"><div class="sensor-big-value"> ${c?t`<span class="sensor-num">${p}</span>${o?t`<span class="sensor-unit">${o}</span>`:""}`:t`<span class="sensor-state-text">${a}</span>`}
            </div>
            <div class="t-tip">
              <div class="t-tip-title"><ha-icon icon="mdi:information-outline"></ha-icon>${d.title}</div>
              <div class="t-tip-desc">${d.desc}</div>
              ${d.ideal?t`<div class="t-tip-ideal"><ha-icon icon="mdi:target"></ha-icon>${d.ideal}</div>`:""}
            </div>
          </div>
        </div>
      </ha-card>
    `}renderCoverCard(e=this.config){const i=e.cover_entity||e.entity||this._buildEntityId("cover","cover"),a=this.hass.states[i];if(!a)return t`<ha-card><div class="error-state"><div class="error-icon"><ha-icon icon="mdi:alert-circle-outline"></ha-icon></div><div class="error-info"><span class="error-title">Cover nicht gefunden</span><span class="error-entity">${i}</span></div></div></ha-card>`;const s=a.state,o=e.name||a.attributes.friendly_name||"Pool Abdeckung",r=a.attributes.current_position??("open"===s?100:0),n="opening"===s||"closing"===s,c=this._getAccentColor("cover",e),l="open"===s?"var(--vpc-success,#34C759)":"closed"===s?c:n?"var(--vpc-warning,#FF9F0A)":"var(--vpc-text-secondary)",d="open"===s?"on":"closed"===s?"off":"auto";return t` <ha-card class="${this._getCardClasses("open"===s||n,e)}" style="--card-accent: ${c}" @click="${()=>this._showMoreInfo(i)}"><div class="accent-bar"></div><div class="card-content"><div class="header"><div class="header-icon ${"closed"!==s?"icon-active":""}" style="--icon-accent: ${c}"> ${e.icon?t`<ha-icon icon="${e.icon}"></ha-icon>`:t`<ha-icon icon="${"open"===s?"mdi:window-shutter-open":"mdi:window-shutter"}"></ha-icon>`}
            </div>
            <div class="header-info">
              <span class="name">${o}</span>
              <span class="header-subtitle" style="color:${l}">${{open:"Geöffnet",closed:"Geschlossen",opening:"Öffnet…",closing:"Schließt…",stopped:"Gestoppt"}[s]||s}</span>
            </div>
            ${!1!==e.show_state?t`<status-badge .state="${d}" .pulse="${n}"></status-badge>`:""}
          </div>

          <!-- Animated pool visualization -->
          <div class="cover-visual">
            ${function(e,i,a){const s=Math.round(72*(1-Math.max(0,Math.min(100,e))/100));return t` <svg viewBox="0 0 90 48" style="width:100%;height:auto;display:block" xmlns="http://www.w3.org/2000/svg"><!-- Pool shell --><rect x="2" y="4" width="76" height="40" rx="9" fill="rgba(0,160,255,0.1)" stroke="${a}" stroke-width="1.5" stroke-opacity="0.35"/><!-- Water texture --><path d="M8,19 C17,15 27,23 37,19 C47,15 57,23 67,19 C71,17 74,19 77,19" fill="none" stroke="rgba(0,150,255,0.45)" stroke-width="1.4" stroke-linecap="round"/><path d="M8,29 C17,25 27,33 37,29 C47,25 57,33 67,29 C71,27 74,29 77,29" fill="none" stroke="rgba(0,150,255,0.25)" stroke-width="1.1" stroke-linecap="round"/><!-- Cover panel --> ${s>0?t`
        <rect x="2" y="4" width="${s}" height="40" rx="9"
              fill="${a}" fill-opacity="0.82"/>
        ${Array.from({length:Math.min(Math.floor(s/7),11)},(e,i)=>t` <line x1="${9+7*i}" y1="6" x2="${9+7*i}" y2="42" stroke="white" stroke-width="0.6" stroke-opacity="0.22"/> `)}
      `:""}
      <!-- Motor / reel housing -->
      <rect x="80" y="8" width="8" height="32" rx="4"
            fill="${a}" fill-opacity="0.45"/>
      <circle cx="84" cy="24" r="5" fill="${a}" fill-opacity="${i?.9:.35}"
              style="${i?"animation:rotate 1.2s linear infinite":""}"/>
      <circle cx="84" cy="24" r="2" fill="white" fill-opacity="${i?.8:.4}"/>
    </svg>`}(r,n,c)}
          </div>

          <!-- Position row -->
          <div class="info-row">
            <ha-icon icon="${n?"mdi:rotate-3d-variant":"mdi:percent"}" style="--mdc-icon-size:17px"></ha-icon>
            <span class="info-label">Position</span>
            <span class="info-value" style="color:${l}">${Math.round(r)}%</span>
            ${n?t`<span class="cover-moving-pill">${"opening"===s?"▲ Öffnet":"▼ Schließt"}</span>`:""}
          </div>

          <!-- Position bar -->
          <div class="cover-pos-bar">
            <div class="cover-pos-fill" style="width:${r}%;background:${c}"></div>
          </div>

          <!-- Position slider -->
          ${!1!==e.show_controls?t` <slider-control label="Position" min="0" max="100" step="5" .value="${Math.round(r)}" unit="%" @value-changed="${e=>{e.stopPropagation(),this.hass.callService("cover","set_cover_position",{entity_id:i,position:e.detail.value})}}" ></slider-control> `:""}

          <!-- Open / Stop / Close buttons -->
          ${!1!==e.show_controls?t` <div class="cover-controls"><button class="cover-btn cover-btn-open ${"open"===s?"cvr-active":""}" @click="${e=>{e.stopPropagation(),this.hass.callService("cover","open_cover",{entity_id:i})}}"><ha-icon icon="mdi:arrow-up" style="--mdc-icon-size:17px"></ha-icon><span>Öffnen</span></button><button class="cover-btn cover-btn-stop ${"stopped"===s?"cvr-active":""}" @click="${e=>{e.stopPropagation(),this.hass.callService("cover","stop_cover",{entity_id:i})}}"><ha-icon icon="mdi:stop" style="--mdc-icon-size:17px"></ha-icon><span>Stop</span></button><button class="cover-btn cover-btn-close ${"closed"===s?"cvr-active":""}" @click="${e=>{e.stopPropagation(),this.hass.callService("cover","close_cover",{entity_id:i})}}"><ha-icon icon="mdi:arrow-down" style="--mdc-icon-size:17px"></ha-icon><span>Schließen</span></button></div> `:""}
        </div>
      </ha-card>`}renderLightCard(e=this.config){const i=e.light_entity||e.entity||this._buildEntityId("light","light"),a=this.hass.states[i];if(!a)return t`<ha-card><div class="error-state"><div class="error-icon"><ha-icon icon="mdi:alert-circle-outline"></ha-icon></div><div class="error-info"><span class="error-title">Licht nicht gefunden</span><span class="error-entity">${i}</span></div></div></ha-card>`;const s=a.state,o="on"===s,r=e.name||a.attributes.friendly_name||"Pool Licht",n=a.attributes.brightness??128,c=Math.round(n/255*100),l=a.attributes.rgb_color??null,d=l?`rgb(${l[0]},${l[1]},${l[2]})`:null,p=d||e.accent_color||this._getAccentColor("light",e),h=a.attributes.color_temp,v=a.attributes.effect;return t` <ha-card class="${this._getCardClasses(o,e)}" style="--card-accent: ${p}" @click="${()=>this._showMoreInfo(i)}"><div class="accent-bar" style="${o&&l?`background:${d};opacity:1`:""}"></div>
        <div class="card-content">
          <div class="header">
            <div class="header-icon ${o?"icon-active":""}" style="--icon-accent: ${p}">
              ${e.icon?t`<ha-icon icon="${e.icon}"></ha-icon>`:function(e,i,a,s){const o=i?`rgb(${i[0]},${i[1]},${i[2]})`:s,r=e?a/255:0;return t` <svg viewBox="0 0 48 48" style="width:100%;height:100%;display:block;overflow:visible" xmlns="http://www.w3.org/2000/svg"> ${e?t`
        <circle cx="24" cy="24" r="23" fill="${o}" fill-opacity="${.07+.18*r}"
                style="animation:light-glow 2.6s ease-in-out infinite"/>
        <circle cx="24" cy="24" r="17" fill="${o}" fill-opacity="${.09+.12*r}"/>
        <g style="transform-origin:24px 24px;animation:spin-slow 9s linear infinite">
          ${[0,60,120,180,240,300].map(e=>t` <line x1="24" y1="5" x2="24" y2="9" stroke="${o}" stroke-width="2.2" stroke-linecap="round" stroke-opacity="${.4+.4*r}" style="transform:rotate(${e}deg);transform-origin:24px 24px"/> `)}
        </g>
      `:t`<circle cx="24" cy="24" r="22" fill="${s}" fill-opacity="0.07"/>`}
      <!-- Housing ring -->
      <circle cx="24" cy="24" r="20" fill="none" stroke="${e?o:s}"
              stroke-width="1.5" stroke-opacity="${e?.55:.3}"/>
      <!-- LED lens -->
      <circle cx="24" cy="24" r="12" fill="${e?o:s}"
      ...
      <!-- Mount points -->
      <circle cx="24" cy="6" r="2" fill="${s}" fill-opacity="0.3"/>
      <circle cx="24" cy="42" r="2" fill="${s}" fill-opacity="0.3"/>
    </svg>`}(o,l,n,p)}
            </div>
            <div class="header-info">
              <span class="name">${r}</span>
              <span class="header-subtitle" style="${o?"color:"+p:""}">
                ${o?v||c+"%":"Aus"}
              </span>
            </div>
            ${!1!==e.show_state?t`<status-badge .state="${s}" .pulse="${o}"></status-badge>`:""}
          </div>

          ${o?t` <!-- Color swatch with interactive color picker --> ${l?t`
              <div class="light-color-swatch" style="background:${d};box-shadow:0 6px 24px ${d}50">
                <span class="light-color-label">RGB ${l[0]}, ${l[1]}, ${l[2]}</span>
                <span class="light-color-hint">Farbe wählen</span>
                <input type="color"
                       .value="${"#"+l.map(e=>e.toString(16).padStart(2,"0")).join("")}"
                       @click="${e=>e.stopPropagation()}"
                       @change="${e=>{e.stopPropagation();const t=e.target.value,a=parseInt(t.slice(1,3),16),s=parseInt(t.slice(3,5),16),o=parseInt(t.slice(5,7),16);this.hass.callService("light","turn_on",{entity_id:i,rgb_color:[a,s,o]})}}"/>
              </div>
            `:h?t` <div class="light-color-swatch" style="background:linear-gradient(135deg,#ffe8a0,#fff3d4);box-shadow:0 6px 24px rgba(255,220,100,0.4)"><span class="light-color-label" style="color:#8B6914">Warmweiß · ${h} Mirek</span></div> `:""}
            ...
            <div class="cover-pos-bar">
              <div class="cover-pos-fill" style="width:${c}%;background:${p}"></div>
            </div>

            ${!1!==e.show_controls?t` <div class="light-brightness-row"><slider-control label="Helligkeit" min="1" max="100" step="5" .value="${c}" unit="%" @value-changed="${e=>{e.stopPropagation(),this.hass.callService("light","turn_on",{entity_id:i,brightness_pct:e.detail.value})}}" ></slider-control></div> `:""}
          `:""}

          ${!1!==e.show_controls?t` <div class="cover-controls"><button class="cover-btn cover-btn-open ${o?"cvr-active":""}" style="--cvr-btn-color:${p}" @click="${e=>{e.stopPropagation(),this.hass.callService("light","turn_on",{entity_id:i})}}"><ha-icon icon="mdi:lightbulb-on" style="--mdc-icon-size:17px"></ha-icon><span>An</span></button><button class="cover-btn cover-btn-close ${o?"":"cvr-active"}" @click="${e=>{e.stopPropagation(),this.hass.callService("light","turn_off",{entity_id:i})}}"><ha-icon icon="mdi:lightbulb-off" style="--mdc-icon-size:17px"></ha-icon><span>Aus</span></button></div> `:""}
        </div>
      </ha-card>`}renderFilterCard(e=this.config){const i=e.filter_pressure_entity||e.entity||this._buildEntityId("sensor","filter_pressure"),a=e.filter_entity,s=e.backwash_entity,o=this.hass.states[i];if(!o)return t`<ha-card><div class="error-state"><div class="error-icon"><ha-icon icon="mdi:alert-circle-outline"></ha-icon></div><div class="error-info"><span class="error-title">Filter nicht gefunden</span><span class="error-entity">${i}</span></div></div></ha-card>`;const r=parseFloat(o.state)||0,n=o.attributes.unit_of_measurement||"bar",c=e.name||o.attributes.friendly_name||"Filter",l=this._getAccentColor("filter",e),d=a?this.hass.states[a]:void 0,p=s?this.hass.states[s]:void 0,h=!d||"on"===d.state,v=!!p&&"on"===p.state,g=r<.5?"var(--vpc-text-secondary)":r<1.2?"var(--vpc-success,#34C759)":r<1.6?"var(--vpc-warning,#FF9F0A)":"var(--vpc-danger,#FF3B30)",u=r<.5?"Niedrig":r<1.2?"Normal":r<1.6?"Erhöht – bald rückspülen":"Kritisch – sofort rückspülen!",m=Math.min(100,r/3*100),f=Math.PI-m/100*Math.PI,x=m>=99.5?"109.9":(60+50*Math.cos(f)).toFixed(1),b=m>=99.5?"60.0":(60-50*Math.sin(f)).toFixed(1),y=m>50?1:0,w="44.5",$="12.4",_="65.2",k="10.3";return t` <ha-card class="${this._getCardClasses(h,e)}" style="--card-accent: ${l}" @click="${()=>this._showMoreInfo(i)}"><div class="accent-bar"></div><div class="card-content"><div class="header"><div class="header-icon ${h?"icon-active":""}" style="--icon-accent: ${l}"> ${e.icon?t`<ha-icon icon="${e.icon}"></ha-icon>`:t`<ha-icon icon="${v?"mdi:rotate-right":"mdi:filter"}" class="${v?"pump-running":""}"></ha-icon>`}
            </div>
            <div class="header-info">
              <span class="name">${c}</span>
              <span class="header-subtitle" style="${v?"color:var(--vpc-warning,#FF9F0A)":""}">
                ${v?"Rückspülung läuft…":h?"Aktiv":"Aus"}
              </span>
            </div>
            ${!1!==e.show_state?t`<status-badge .state="${h?"on":"off"}" .pulse="${v}"></status-badge>`:""}
          </div>
          ...
          <div class="filter-gauge-wrap">
            <svg viewBox="0 0 120 68" style="width:100%;max-height:90px;display:block;overflow:visible">
              ...
              <path d="M ${"16.7"},${"35.0"} A 50,50 0 0,1 ${w},${$}" fill="none" stroke="rgba(52,199,89,0.22)" stroke-width="10"/>
              <!-- Elevated zone (1.2–1.6 bar) -->
              <path d="M ${w},${$} A 50,50 0 0,1 ${_},${k}" fill="none" stroke="rgba(255,159,10,0.22)" stroke-width="10"/>
              <!-- Critical zone (>1.6 bar) -->
              <path d="M ${_},${k} A 50,50 0 0,1 109.9,60" fill="none" stroke="rgba(255,59,48,0.22)" stroke-width="10"/>
              <!-- Value arc -->
              ${m>1?t` <path d="M 10,60 A 50,50 0 ${y},1 ${x},${b}" fill="none" stroke="${g}" stroke-width="10" stroke-linecap="round"/> `:""}
              <!-- Tick marks at 0, 1, 2, 3 bar -->
              <line x1="10" y1="58" x2="10" y2="62" stroke="rgba(120,120,128,0.4)" stroke-width="1.5"/>
              <line x1="${"35.0"}" y1="${"16.7"}" x2="${"37.0"}" y2="${"20.2"}" stroke="rgba(120,120,128,0.4)" stroke-width="1.5"/>
              <line x1="60" y1="10" x2="60" y2="14" stroke="rgba(120,120,128,0.4)" stroke-width="1.5"/>
              <line x1="${"85.0"}" y1="${"16.7"}" x2="${"83.0"}" y2="${"20.2"}" stroke="rgba(120,120,128,0.4)" stroke-width="1.5"/>
              ...
              <text x="60" y="52" text-anchor="middle" font-size="20" font-weight="700"
                    style="fill:${g};letter-spacing:-0.5px">${r.toFixed(1)}</text>
              <text x="60" y="63" text-anchor="middle" font-size="9" font-weight="500"
                    style="fill:rgba(110,110,120,0.8)">${n}</text>
              <!-- Scale labels -->
          ...
          <div class="config-section">
            ...
            <ha-entity-picker
              label="${e?"cover"===this._config.card_type?"Cover Entität":"Licht Entität":"Entität"}"
              .hass="${this.hass}"
        ...
        ${["pump","heater","solar","dosing","overview","system","chemical"].includes(this._config.card_type)?t` <details class="advanced-section"><summary><ha-icon icon="mdi:swap-horizontal"></ha-icon><span>Entitäten überschreiben</span></summary><div class="advanced-content"> ${"pump"===this._config.card_type||"overview"===this._config.card_type||"system"===this._config.card_type?t`
              ...
              `:""}
              ${"heater"===this._config.card_type||"overview"===this._config.card_type||"system"===this._config.card_type?t` <ha-entity-picker label="Heizung (override)" .hass="${this.hass}" .value="${this._config.heater_entity||""}" .includeDomains="${["climate"]}" @value-changed="${e=>this._overrideChanged("heater_entity",e.detail.value)}" allow-custom-entity></ha-entity-picker> `:""}
              ${"solar"===this._config.card_type||"overview"===this._config.card_type||"system"===this._config.card_type?t` <ha-entity-picker label="Solar (override)" .hass="${this.hass}" .value="${this._config.solar_entity||""}" .includeDomains="${["climate"]}" @value-changed="${e=>this._overrideChanged("solar_entity",e.detail.value)}" allow-custom-entity></ha-entity-picker> `:""}
              ${"dosing"===this._config.card_type||"overview"===this._config.card_type||"system"===this._config.card_type?t` <ha-entity-picker label="Chlor-Dosierung (override)" .hass="${this.hass}" .value="${this._config.chlorine_entity||""}" .includeDomains="${["switch"]}" @value-changed="${e=>this._overrideChanged("chlorine_entity",e.detail.value)}" allow-custom-entity></ha-entity-picker> `:""}
              ${["dosing","overview","system","chemical"].includes(this._config.card_type)?t` <ha-entity-picker label="pH-Sensor (override)" .hass="${this.hass}" .value="${this._config.ph_value_entity||""}" .includeDomains="${["sensor"]}" @value-changed="${e=>this._overrideChanged("ph_value_entity",e.detail.value)}" allow-custom-entity></ha-entity-picker><ha-entity-picker label="ORP-Sensor (override)" .hass="${this.hass}" .value="${this._config.orp_value_entity||""}" .includeDomains="${["sensor"]}" @value-changed="${e=>this._overrideChanged("orp_value_entity",e.detail.value)}" allow-custom-entity></ha-entity-picker> `:""}
              ${["heater","solar","overview","system","chemical"].includes(this._config.card_type)?t` <ha-entity-picker label="Pool-Temperatur (override)" .hass="${this.hass}" .value="${this._config.pool_temp_entity||""}" .includeDomains="${["sensor"]}" @value-changed="${e=>this._overrideChanged("pool_temp_entity",e.detail.value)}" allow-custom-entity></ha-entity-picker> `:""}
            </div>
          </details>
        `:""}
