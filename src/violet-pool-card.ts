...
interface LovelaceCardConfig {
  ...
  entity?: string;
  entities?: string[];
  card_type: 'pump' | 'heater' | 'solar' | 'dosing' | 'overview' | 'compact' | 'system' | 'details';
  name?: string;
  icon?: string;
...
export class VioletPoolCard extends LitElement {
  ...
  public setConfig(config: VioletPoolCardConfig): void {
    ...
    if (config.card_type !== 'overview' && config.card_type !== 'system' && config.card_type !== 'details' && !config.entity) {
    ...
    return html` <ha-card class="${this._getCardClasses(isRunning, config)}" style="--card-accent: ${accentColor}" @click="${() => this._showMoreInfo(config.entity!)}" ><div class="accent-bar"></div><div class="card-content"><div class="header"><div class="header-icon ${isRunning ? 'icon-active' : ''}" style="--icon-accent: ${accentColor}"><ha-icon icon="${config.icon || 'mdi:pump'}" class="${isRunning ? 'pump-running' : ''}" ></ha-icon></div><div class="header-info"><span class="name">${name}</span><span class="header-subtitle" style="${isRunning ? `color: ${speedColor.color}` : ''}">
          ...
          <div class="speed-segments-container">
            <div class="speed-segments">
              ${[1, 2, 3].map(level => html` <button class="speed-segment ${currentSpeed === level ? 'seg-active' : currentSpeed > level ? 'seg-past' : ''}" style="--seg-color: ${speedColors[level]}" @click="${(e: Event) => { e.stopPropagation(); const sc = new ServiceCaller(this.hass); sc.setPumpSpeed(config.entity!, level); }}" title="${speedLabels[level]}" ><ha-icon icon="${speedIcons[level]}" style="--mdc-icon-size: 14px"></ha-icon><span>${speedLabels[level]}</span></button> `)}
            </div>
            <button
  ...
  private renderHeaterCard(config: VioletPoolCardConfig = this.config): TemplateResult {
    ...
    return html` <ha-card class="${this._getCardClasses(isHeating, config)}" style="--card-accent: ${accentColor}" @click="${() => this._showMoreInfo(config.entity!)}" ><div class="accent-bar"></div><div class="card-content"><div class="header"><div class="header-icon ${isHeating ? 'icon-active' : ''}" style="--icon-accent: ${accentColor}"><ha-icon icon="${config.icon || 'mdi:radiator'}" class="${isHeating ? 'heater-active' : ''}" ></ha-icon></div><div class="header-info"><span class="name">${name}</span><span class="header-subtitle">${this._getFriendlyState(state)}</span></div> ${config.show_state ? html`<status-badge .state="${state}"></status-badge>`
          ...
          ${currentTemp !== undefined
            ? html` <div class="temp-hero" style="--temp-color: ${tempColor?.color || 'var(--vpc-primary)'}"><div class="temp-hero-main"><span class="temp-hero-value">${currentTemp.toFixed(1)}</span><span class="temp-hero-unit">°C</span></div> ${targetTemp !== undefined ? html`
          ...
          ${outsideTemp !== undefined
            ? html` <div class="info-row ${isBlockedByOutsideTemp ? 'info-row-warning' : ''}"><ha-icon icon="mdi:thermometer"></ha-icon><span class="info-label">Outside</span><span class="info-value">${outsideTemp.toFixed(1)}°C</span> ${isBlockedByOutsideTemp ? html`<span class="info-badge warning">Min ${minOutsideTemp}°C</span>`
                    : ''}
                </div>
              `
            : ''}
          ...
          <div class="solar-temps">
            ...
            <div class="solar-temp-comparison">
              ${poolTemp !== undefined
                ...
                : ''}
              ${tempDelta !== undefined
                ? html` <div class="solar-delta-badge ${tempDelta >= 3 ? 'delta-great' : tempDelta > 0 ? 'delta-ok' : 'delta-low'}"><ha-icon icon="${tempDelta >= 0 ? 'mdi:trending-up' : 'mdi:trending-down'}" style="--mdc-icon-size: 16px"></ha-icon><span>${tempDelta > 0 ? '+' : ''}${tempDelta.toFixed(1)}°C</span></div> `
                : ''}
  ...
  private renderDosingCard(config: VioletPoolCardConfig = this.config): TemplateResult {
    ...
    return html` <ha-card class="${this._getCardClasses(isDosing, config)}" style="--card-accent: ${accentColor}" @click="${() => this._showMoreInfo(config.entity!)}" ><div class="accent-bar"></div><div class="card-content"><div class="header"><div class="header-icon ${isDosing ? 'icon-active' : ''}" style="--icon-accent: ${accentColor}"><ha-icon icon="${config.icon || this._getDosingIcon(dosingType)}" class="${isDosing ? 'dosing-active' : ''}" ></ha-icon></div><div class="header-info"><span class="name">${name}</span><span class="header-subtitle">${this._getFriendlyState(state)}</span></div> ${config.show_state ? html`<status-badge .state="${state}" .pulse="${isDosing}"></status-badge>`
          ...
          ${currentValue !== undefined
            ? html` <!-- Dosing value hero with progress bar --><div class="dosing-value-block"><div class="dosing-value-row"><div class="dosing-value-main" style="color: ${valueColor?.color || 'var(--vpc-text)'}"><span class="dosing-label-tag">${dosingLabel}</span><span class="dosing-current-value">${currentValue.toFixed(decimals)}</span><span class="dosing-current-unit">${unit}</span></div><div class="dosing-status-pill" style="background: ${valueColor?.color ? valueColor.color + '18' : 'rgba(0,0,0,0.05)'}; color: ${valueColor?.color || 'var(--vpc-text-secondary)'}"> ${valueStatusLabel} </div></div> ${valuePct !== undefined ? html`
  ...
  private renderOverviewCard(config: VioletPoolCardConfig = this.config): TemplateResult {
    ...
    return html` <ha-card class="${this._getCardClasses(anyActive, config)}" style="--card-accent: ${accentColor}" ><div class="accent-bar"></div><div class="card-content"><!-- Header --><div class="header"><div class="header-icon ${anyActive ? 'icon-active' : ''}" style="--icon-accent: ${accentColor}"><ha-icon icon="mdi:pool"></ha-icon></div><div class="header-info"><span class="name">${name}</span><span class="header-subtitle"> ${anyActive ? `${activeCount} device${activeCount !== 1 ? 's' : ''} active` : 'All systems idle'}
          ...
          <div class="chemistry-grid">
            ${poolTemp !== undefined
              ? html` <div class="chemistry-card" style="--chem-color: ${tempColor?.color || '#4CAF50'}" @click="${(e: Event) => { e.stopPropagation(); this._showMoreInfo(poolTempSensorId); }}"><div class="chem-icon-wrap"><ha-icon icon="mdi:thermometer-water"></ha-icon></div><span class="chemistry-val">${poolTemp.toFixed(1)}°</span><span class="chemistry-unit">°C</span><span class="chemistry-label">Temp</span> ${tempPct !== undefined ? html`<div class="chem-mini-bar"><div class="chem-mini-fill" style="width: ${tempPct}%; background: ${tempColor?.color || '#4CAF50'}"></div></div>`
                      : ''}
                  </div>
                `
              : ''}
            ${phValue !== undefined
              ? html` <div class="chemistry-card" style="--chem-color: ${phColor?.color || '#4CAF50'}" @click="${(e: Event) => { e.stopPropagation(); this._showMoreInfo(phSensorId); }}"><div class="chem-icon-wrap"><ha-icon icon="mdi:ph"></ha-icon></div><span class="chemistry-val">${phValue.toFixed(1)}</span><span class="chemistry-unit">pH</span><span class="chemistry-label">${phStatus === 'ok' ? 'Optimal' : 'Attention'}</span> ${phPct !== undefined ? html`
              ...
              : ''}
            ${orpValue !== undefined
              ? html` <div class="chemistry-card" style="--chem-color: ${orpColor?.color || '#4CAF50'}" @click="${(e: Event) => { e.stopPropagation(); this._showMoreInfo(orpSensorId); }}"><div class="chem-icon-wrap"><ha-icon icon="mdi:lightning-bolt"></ha-icon></div><span class="chemistry-val">${orpValue.toFixed(0)}</span><span class="chemistry-unit">mV</span><span class="chemistry-label">${orpStatus === 'ok' ? 'Optimal' : orpStatus === 'warning' ? 'Low' : 'High'}</span> ${orpPct !== undefined ? html`
                          <div class="chem-mini-bar">
