 ...
 *
 * Component: Slider Control – Universeller Schieberegler zur Werteinstellung (z.B. Temperatur, Drehzahl)
...
export class SliderControl extends LitElement {
  ...
  private debounceDelay = 150; // ms - Reduced for better responsiveness
  ...
  protected render(): TemplateResult {
    const range = this.max - this.min;
    const percentage = range > 0 ? Math.min(100, Math.max(0, ((this.localValue - this.min) / range) * 100)) : 0;
        ...
        ${allLabels.length > 0
          ? html` <div class="labels"> ${allLabels.map( (item) => html`
                    <span
                      ...
                      title="${item.label}"
                      @click="${() => !this.disabled && this.handleChange({ target: { value: String(item.value) } } as unknown as Event)}"
                    >
                      ${item.label}
                    </span>
                  `
